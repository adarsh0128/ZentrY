...
export const PasswordProvider = ({ children }) => {
  ...
  const Postpassword = async (data) => {
    ...
    try {
      const res = await axios.post(
        "http://localhost:5000/pwm/api/password/postpassword",
        data,
        {
          headers: {
  ...
  const Getpassword = async () => {
    try {
      const res = await axios.get(
        "http://localhost:5000/pwm/api/password/getpassword",
  ...
  const Deletepassword = async (pwid) => {
    try {
      const res = await axios.delete(
        `http://localhost:5000/pwm/api/password/deletepassword/${pwid}`,
  ...
  const Updatepassword = async ({ prevvalue, newpwddata }) => {
    ...
    try {
      const res = await axios.put(
        `http://localhost:5000/pwm/api/password/updatepassword/${prevvalue?._id}`,
        newpwddata,
  ...
  const LogoutUser = async () => {
    try {
      const res = await axios.get("http://localhost:5000/pwm/api/user/logout", {
        headers: {
          "auth-token": localStorage.getItem("token"),
        },
      });
